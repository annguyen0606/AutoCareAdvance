...
{
    public partial class FrmPhieuSuaChuaThangLoi : Form
    {
        ...
        public FrmPhieuSuaChuaThangLoi()
        {
            InitializeComponent();
        }
        ...
        private async void FrmPhieuSuaChuaThangLoi_Load(object sender, EventArgs e)
        {
            ...
            SqlCommand cmd = new SqlCommand("sp_Test_Report3_1", con);
            ...
            DataTable dtPhuTungReal = ReReverseRowsInDataTable1(dtPhuTung,index);//test by duocnv
        ...
        private void FrmPhieuSuaChuaThangLoi_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private DataTable ReReverseRowsInDataTable1(DataTable inputTable,int iDem)
        {
            DataTable outputTable = inputTable.Clone();
            for (int i = inputTable.Rows.Count - 1; i >= inputTable.Rows.Count-iDem; i--)
            {
                outputTable.ImportRow(inputTable.Rows[i]);
            }
            for(int i=0;i< inputTable.Rows.Count - iDem;i++)
            {
                outputTable.ImportRow(inputTable.Rows[i]);
            }
